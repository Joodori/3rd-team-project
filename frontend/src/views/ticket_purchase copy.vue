<template>
  ...
  <div class="container d-flex flex-column w-100 h-25"> <!--라디오버든 1개만 선택-->
    ...
    <label class="from-check-label" for="tiketFree">
      자유이용권 35000원
    </label>
    </div>
    <div class="form-check">
      ...
      v-model="selectedTicketPrice"/>
    <label class="from-check-label" for="tiketAfternoon"><!--라디오 선택 항목-->
      오후권 20000원
    </label>
    </div>
    ...
    <div class="form-check">
      ...
      v-model="selectedTicketPrice"/>
    <label class="from-check-label" for="tiketVip">
      vip이용권 40000원
    </label>
    </div>
    <div class="form-check">
      ...
      v-model="selectedTicketPrice"/>
    <label class="from-check-label" for="tiketVvip">
      vvip이용권 50000원
    </label>
    </div>
  </div>
...
const handleReservation = () => {
  ...
  const reservationData = {
    date: formattedDate, // <--- 팀원이 요청한 'YYYY-MM-DD' 문자열
    count: personCount.value,
    totalCost: selectedTicketPrice.value * personCount.value
  };

  console.log("DB로 전송할 데이터:", reservationData);
